...
var ScrollbarCarousel = function (_Component) {
  ...
  function ScrollbarCarousel(props) {
    ...
    _this.setScrollInfo = function () {
      _this.slideWidth = _this.el.scrollWidth / _this.props.slides.length; // width of individual slide item
      _this.scrollPos = _this.el.scrollLeft; // current scroll position
      _this.slideNum = _this.scrollPos / _this.slideWidth; // current slide num (zero index, float)

      // internal slideNum (float) is used to show/hide chevrons only
      _this.setState({
        slideNum: Math.floor(_this.slideNum + 0.51) // used as halfway point threshold
      });
    };
  ...
  _createClass(ScrollbarCarousel, [{
    ...
    value: function handleNavClick(dir) {
      ...
      return function (e) {
        ...
        var slideIncr = _this3.isTweening ? slidesShown : 0; // increment if user keeps clicking in mid-tween
        var nextSlideNum = Math.round(_this3.slideNum + (slidesShown + slideIncr - 0.49) * dir);
        _this3.animateToSlide(nextSlideNum);
      };
    }
    ...
    value: function animateToSlide(slideNum) {
      ...
      var animateScroll = function animateScroll() {
        _this4.reqAnimFrame = window.requestAnimationFrame(function () {
          var nextTween = (0, _utils.getNextTween)(tweenPos, fromPos, toPos, 300, easingFn);
